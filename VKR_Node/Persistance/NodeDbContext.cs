...
namespace VKR_Node.Persistance
{
    ...
    public class NodeDbContext : DbContext
    {
        ...
        public DbSet<FileMetadataEntity> FilesMetadata { get; set; } = null!;
        public DbSet<ChunkMetadataEntity> ChunksMetadata { get; set; } = null!;
        public DbSet<ChunkLocationEntity> ChunkLocations { get; set; } = null!;
        public DbSet<NodeStateEntity> NodeStates { get; set; } = null!;
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // --- FileMetadataEntity Configuration (No changes needed) ---
            ...
            // --- ChunkMetadataEntity Configuration (No changes needed) ---
            modelBuilder.Entity<ChunkMetadataEntity>(entity =>
            {
                entity.ToTable("ChunksMetadata");
                entity.HasKey(e => new { e.FileId, e.ChunkId }); // Consider changing key if ChunkId is unique
                entity.HasMany(e => e.Locations)
                      .WithOne(l => l.ChunkMetadataEntity)
                      .HasForeignKey(l => new { l.FileId, l.ChunkId })
                      .IsRequired()
                      .OnDelete(DeleteBehavior.Cascade);
            });

            // --- ChunkLocationEntity Configuration (No changes needed) ---
            modelBuilder.Entity<ChunkLocationEntity>(entity =>
            {
                entity.ToTable("ChunkLocations");
                entity.HasKey(e => new { e.FileId, e.ChunkId, e.StoredNodeId }); // Or potentially (ChunkId, StoredNodeId)
                entity.HasIndex(e => e.StoredNodeId);
            });

            // --- NodeStateEntity Configuration (Ensure new properties are handled if needed) ---
            modelBuilder.Entity<NodeStateEntity>(entity =>
            {
                entity.ToTable("NodeStates");
                entity.HasKey(e => e.NodeId);
                entity.Property(e => e.Address).IsRequired();
                entity.Property(e => e.State).IsRequired();
                // EF Core typically handles nullable types (long?, DateTime?) correctly for columns
                // No specific configuration needed for the new properties unless you want constraints
            });

            // --- UserEntity Configuration (ADD THIS) ---
            modelBuilder.Entity<UserEntity>(entity =>
            {
                 ...
                 entity.Property(e => e.CreationTime).HasDefaultValueSql("CURRENT_TIMESTAMP"); // Use DB default time
